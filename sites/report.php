...
<body>
	...
	$sql = 'select * from assets inner join locations on assets.rtd_location_id = locations.id where rtd_location_id = "" or rtd_location_id is null and deleted_at is null and asset_tag not in (select name from tempExclusions) and serial not in (select name from tempExclusions);';
